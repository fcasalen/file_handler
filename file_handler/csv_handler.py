from pandas import read_csv, DataFrame, NA
...
class CSVHandler:
    ...
    def load(file_path_with_password:tuple[str, str], encoding:str, mode:str = 'r'):
        ...
        try:
            return {'Unique': read_csv(file_path).fillna(NA)}
    ...
    def write(file_path:str, encoding:str, data:DataFrame|dict[str, DataFrame], mode:str = 'w'):
        ...
        if isinstance(data, DataFrame):
            data.fillna(NA).to_csv(file_path, index=False)
            return
        if len(data.keys()) > 1:
            for k,v in data.items():
                v.fillna(NA).to_csv(f'{file_path.replace(".csv", "")}_{k}.csv', index=False)
        else:
